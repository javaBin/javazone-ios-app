...
extension WebView: WKNavigationDelegate {
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        Logger.networking.debug("WebView: didFinish: Partners loaded")
...
WebView: didFail: Partners failed to load \(error.localizedDescription, privacy: .public)
"""
        )
    ...
    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        Logger.networking.warning("""
WebView: didFailProvisional: Partners failed to load (provisional) \(error.localizedDescription, privacy: .public)
"""
        )
    ...
    func webView(_ webView: WKWebView,
        ...
        if let response = navigationResponse.response as? HTTPURLResponse {
              if response.statusCode >= 400 {
                  Logger.networking.warning("""
WebView: decidePolicy: Fetch partners got HTTP code \(response.statusCode)
"""
                  )
              }
        }
